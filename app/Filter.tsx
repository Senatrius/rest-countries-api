...
export const Filter = ({filter, setFilter}: {filter: string, setFilter: React.Dispatch<React.SetStateAction<string>>}) => {
  ...
  return <div className="relative md:ml-4 flex-shrink self-start text-[0.875rem]">
      <button type="button" onClick={() => setIsFilterOpen(!isFilterOpen)} className="rounded py-5 px-7 shadow-component bg-white text-lightText dark:bg-darkElement dark:text-white focus:outline-none hover:shadow-lg transition-shadow flex items-center justify-between whitespace-nowrap dark:hover:brightness-125 hover:brightness-95">
