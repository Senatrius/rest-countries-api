...
export const Search = ({value, setQuery}: {value: string, setQuery: React.Dispatch<React.SetStateAction<string>>}) => {
  ...
  return <label className="relative block flex-grow">
    ...
    <input type="text" placeholder="Search for a country..." value={value} onChange={handleChange} className="w-full md:max-w-[30rem] flex basis-full rounded shadow-component text-sm placeholder:text-grey pl-24 py-5 pr-4 dark:placeholder:text-white dark:bg-darkElement focus:outline-none focus:shadow-lg transition-shadow" />
  </label>
}